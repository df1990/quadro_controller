...
/*! \brief Reinitialisation of PID controller parameters, changes values of P,I,D factors but leaves sumError.
 *
 *  Initialise the variables used by the PID algorithm.
 *
 *  \param p_factor  Proportional term.
...
void pid_Reinit(int16_t p_factor, int16_t i_factor, int16_t d_factor, struct PID_DATA *pid)
{
  // Tuning constants for PID loop
  pid->P_Factor = p_factor;
  pid->I_Factor = i_factor;
  pid->D_Factor = d_factor;
  // Limits to avoid overflow
  pid->maxError = INT32_MAX / (pid->P_Factor + 1);
  pid->maxSumError = ((INT32_MAX /2) / (pid->I_Factor + 1));
